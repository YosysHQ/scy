...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"name": "fixed_data", "sequence": ["123", "abc"], "cover_stmts": ["", "blank"]},
    ],
    scope="class",
)
class TestFixturesClass:
    def test_sequence(self, test, sequence):
        if "sequence" in test:
            ...
            assert sequence == ["cover cp_4:", " cover cp_12:", " cover cp_14:", "  cover cp_12:"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"name": "good_trace", "sequence": ["cover cp_4:", " trace now:"], "data": ["4"]},
        {
            "name": "trace_final",
            "sequence": ["cover cp_1:", " cover cp_2", "cover cp_3"],
            "args": ["--tracefinal"],
            "data": ["1", "2", "3"],
            "chunks": [1, 1, 3],
        },
        {
            "name": "add_reverse",
            "sequence": ["cover cp_254:", " add assume reverse:", "  cover cp_253"],
            "cover_stmts": [
                "\tif (!reset) begin",
        ...
        {
            "name": "force_reset",
            "sequence": ["cover cp_4:", " cover cp_3", "cover cp_3"],
            "cover_stmts": [
                "\tno_reverse: assume (!reverse);",
                "\tif (!reset) begin",
                "\t\tcp_3: cover(count==3);",
                "\t\tcp_4: cover(count==4);",
                "\tend",
        ...
        {"name": "good_dir", "data": ["1"], "args": ["-f", "-d", "this_dir"], "mkdir": "this_dir"},
        {"name": "empty_tree", "sequence": ["cover cp_4", "", "cover cp_3"], "data": ["4", "3"]},
        {
            "name": "tree_comment",
            "sequence": ["cover cp_4", "#comment", "cover cp_3"],
            "data": ["4", "3"],
        },
        {
            "name": "big_tree",
    ...
    def test_files(self, test: dict[str, str | list[str]], scy_dir: Path, output_dir: Path):
        ...
        if "sequence" in test:
            ...
            for stmt in test["sequence"]:
                ...
                if not (isinstance(task, TaskTree) and task.is_runnable):
                    continue
                if "cover" in stmt:
                    match_str = f"{task.name}.sby"
                elif "trace" in stmt:
                    match_str = f"{task.name}.vcd"
                else:
                    continue
    ...
    def test_chunks(self, test: dict[str, Any], scy_chunks: list[int]):
        if "chunks" in test:
            ...
            if scy_chunks != test["chunks"]:
                pytest.skip(
                    f"for {test['name']}; expected {test['chunks']} chunks, got {scy_chunks}"
                )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {
            "name": "no_cover",
            "sequence": ["cover cp_1:", " cover cp_2"],
            "data": [],
        ...
        {
            "name": "bad_cell",
            "sequence": ["add bad cell", ""],
            "data": [],
            "error": "cell type 'bad'",
        },
        {
            "name": "bad_enable",
            "sequence": ["enable bad cell", ""],
            "data": [],
            "error": "bad sequence",
        },
    ],
...
@pytest.mark.parametrize(
    "test",
    [
        {"name": "baseline", "data": ["1", " 2", "  3"], "args": []},
        {"name": "dump_tree", "data": ["1", " 2", "  3"], "args": ["--dumptree"]},
        {"name": "dump_common", "data": ["1", " 2", "  3"], "args": ["--dumpcommon"]},
        {"name": "setup_mode", "data": ["1", " 2", "  3"], "args": ["--setup"]},
        {
            "name": "tree_before_setup",
            "data": ["1", " 2", "  3"],
            "args": ["--setup", "--dumptree"],
        },
        {
            "name": "common_before_setup",
            "data": ["1", " 2", "  3"],
            "args": ["--setup", "--dumpcommon"],
        },
        {
            "name": "tree_before_common",
            "data": ["1", " 2", "  3"],
            "args": ["--dumptree", "--dumpcommon"],
        },
        {"name": "with_errors", "data": ["1", " 2", "  3"], "args": ["-E"]},
        {"name": "tracefinal_outputs", "data": ["1", " 2", "  3"], "args": ["--tracefinal"]},
        {
            "name": "tracefinal_recovers",
            "data": ["1", " 2", "  3", "  44"],
            "args": ["--tracefinal"],
        },
    ],
...
@pytest.mark.parametrize(
    "test",
    [
        {"name": "base_parse", "data": ["1", " 2", "  3"], "args": ["--dumpcommon"]},
        {
            "name": "add_assume",
            "sequence": ["add assume cell"],
            "data": ["1"],
            "args": ["--dumpcommon"],
        },
        {
            "name": "add_enable",
            "sequence": ["enable cell:", " cover cp_1"],
            "data": ["1"],
            "args": ["--dumpcommon"],
        },
        {
            "name": "add_body_enable",
            "sequence": ["cover cp_1:", " enable this"],
            "data": ["1"],
            "args": ["--dumpcommon"],
        },
        {
            "name": "add_more",
            "sequence": ["add assume cell", "add assert here"],
            "data": ["1"],
            "args": ["--dumpcommon"],
        },
        {
            "name": "add_both",
            "sequence": ["add assume seven:", " disable three:", "  cover cp_2"],
            "data": ["1"],
            "args": ["--dumpcommon"],
        },
    ],
